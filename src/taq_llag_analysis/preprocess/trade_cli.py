...
def build_trade_summary(
    *,
    ...
    """Write filtered trade parquet files for the configured dates.
    ...
    for date_yyyymmdd, symbols in _cli_common.symbols_by_date(target_dates, symbol_prefix).items():
        logger.info(
            "trade CLI started date=%s symbol_prefix=%r",
            date_yyyymmdd,
            symbol_prefix,
        )
        ...
        logger.info(
            "trade CLI completed date=%s symbol_count=%d written=%d",
            date_yyyymmdd,
            len(symbols),
            len(written_paths),
        )
...
def main() -> int:
    """Run the trade parquet-writing CLI.
    ...
    logger.info("trade CLI batch started target_dates=%s", list(TARGET_DATES))
    summary = build_trade_summary()
    logger.info("trade CLI batch completed dates=%d", len(summary))
