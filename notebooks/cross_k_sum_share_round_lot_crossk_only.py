...
def _(mo):
    ...
    overview_section = mo.md(
        ...
        This notebook analyzes a cross-K-only summary by aggregating the
        negative-side and positive-side cross-K windows into totals and
        within-side shares, then comparing observed round-lot transition
        groups around the November 3, 2025 reform.
...
@app.cell
def _(Path, pd):
    input_csv_path = Path("../data/derived/trade_quote_modes/all-crossk/cross_k_summary.csv")
    output_base_dir = Path("output/cross_k_sum_share_round_lot_effects_crossk_only")
    trade_exchange = "Z"
    quote_exchange = "Q"
    date_pre = "20251031"
    date_post = "20251103"
    ...
    raw_panel_columns = list(raw_cross_k_columns)
    outcome_specs = [
        {
...
def _(
    ...
    inputs_section = mo.md(
        ...
        This section records the source cross-K-only CSV, exchange-pair filter,
        analysis dates, and round-lot rules used to build the comparison sample.
...
def _(
    ...
    header_df = pd.read_csv(resolved_input_csv_path, nrows=0)
    required_columns = [
        "date_yyyymmdd",
        "symbol",
        "trade_exchange",
    ...
    missing_columns = [
        column_name for column_name in required_columns if column_name not in header_df.columns
    ]
    if missing_columns:
        missing_text = ", ".join(missing_columns)
        message = (
            "cross_k summary is missing required columns for the cross-K-only "
            f"round-lot notebook: {missing_text}"
        )
...
def _(
    ...
    def derive_aggregated_outcomes(
        ...
        *,
        ...
        """Construct derived outcomes from the raw cross-k columns.
...
def _(mo):
    results_section = mo.md(
        ...
        eight cross-K-derived outcomes.
        """
    )
    results_section
    return
